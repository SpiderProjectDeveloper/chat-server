...
static const std::string _chat_table = "chat";				
static const std::string _images_table = "images";				
...
int chatDbWriteImage_( void* db, const char *activity, const char *user, const char *message, 
...
static int create_table( sqlite3* _db ) {
	...
	r = "CREATE TABLE IF NOT EXISTS " + _images_table + 
		" (image TEXT NOT NULL, width INT, height INT, type TEXT );";
	...
	r = "CREATE TABLE IF NOT EXISTS " + _chat_table + 
		...
		" CONSTRAINT fkImageId FOREIGN KEY(imageId) REFERENCES ROWID(" + _images_table + ") ON DELETE CASCADE);";
	status = make_request( _db, r.c_str(), nullptr, nullptr, nullptr );

	r = "CREATE INDEX 'datetime_index' ON " + _chat_table + " (`datetime` DESC)";
	status = make_request( _db, r.c_str(), nullptr, nullptr, nullptr );

	r = "CREATE INDEX 'activity_index' ON " + _chat_table + " (`activity` DESC)";
	status = make_request( _db, r.c_str(), nullptr, nullptr, nullptr );
	return status;
}
